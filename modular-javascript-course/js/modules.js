...
CORE.create_module('product-panel', function (sb) {
  ...
  return {
    ...
    change_filter : function (filter) {
      reset();
      eachProduct(function (product) {
        if (product.getAttribute('data-8088-keyword').toLowerCase().indexOf(filter.toLowerCase()) === -1) {
    ...
    search : function (query) {
      ...
      eachProduct(function (product) {
        if (product.getElementsByTagName('p')[0].innerHTML.toLowerCase().indexOf(query) === -1) {
          product.style.opacity = '0.2';
        }
      });
    },
